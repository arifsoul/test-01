...
def generate_html_report(data, le_type, le_location):
    ...
    # Manually serialize JSON to avoid Jinja2 parsing issues
    ...
    <body class="bg-gray-100 p-6">
        <div id="root"></div>
        <script>
            const typeChartData = {{ type_chart_json | safe }};
            const locationChartData = {{ location_chart_json | safe }};
            const clusterChartData = {{ cluster_chart_json | safe }};
            const App = () => (
                <div className="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md">
                    ...
                    <div className="mt-6">
                        ...
                        <p>
                            The location <strong>{{ top_location | safe }}</strong> appears most frequently,
    ...
    top_location = clean_string(location_chart_data[0]['location']) if location_chart_data else 'Unknown'
    top_location_count = location_chart_data[0]['count'] if location_chart_data else 0
    
    html_content = template.render(
        type_chart_json=type_chart_json,
        location_chart_json=location_chart_json,
        cluster_chart_json=cluster_chart_json,
