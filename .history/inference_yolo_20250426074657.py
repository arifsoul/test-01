...
class SimpleFruitDetectionGUI(QMainWindow):
    def __init__(self):
        ...
        self.process_frame_interval = 15  # Reduced for more frequent detection
        self.model = None
        self.video_fps = 30
        self.total_frames = 0
        self.duration = 0
        self.average_fps = 0
        self.tracking_threshold = 30  # Adjusted for smaller objects or faster movement
        self.max_frames_missing = 30  # Max frames an object can be missing before removal
        self.velocity_smoothing_alpha = 0.5  # Smoothing factor for velocity
    ...
    def update_frame(self):
        ...
        for track_id, obj in list(tracked_objects.items()):
            ...
            if self.frame_count % self.process_frame_interval == 0:
                # Calculate new velocity from movement
                new_vx = new_center_x - center_x
                new_vy = new_center_y - center_y
                # Apply moving average
                vx = self.velocity_smoothing_alpha * new_vx + (1 - self.velocity_smoothing_alpha) * vx
                vy = self.velocity_smoothing_alpha * new_vy + (1 - self.velocity_smoothing_alpha) * vy
            else:
                # Maintain current smoothed velocity
                vx, vy = vx, vy
