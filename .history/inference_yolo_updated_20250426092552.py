...
def init_kalman_filter(fps=30):
    ...
    kf.R = np.eye(2) * 1.0  # Reduced measurement noise for faster response
    kf.P = np.eye(4) * 100  # Initial state covariance
    # Increased process noise for velocity to allow faster adaptation
    kf.Q = np.diag([0.01 * dt, 0.01 * dt, 0.05 * dt, 0.5 * dt])
    # Initialize with higher vy for faster downward motion
    kf.x = np.array([[0], [0], [0], [5.0]])  # Initial state: [x=0, y=0, vx=0, vy=5]
    return kf
...
class SimpleFruitDetectionGUI(QMainWindow):
    ...
    def update_frame(self):
                    ...
                    for box, track_id, cls, conf in zip(boxes, track_ids, classes, confidences):
                        ...
                        for existing_id, obj in tracked_objects.items():
                            ...
                            if distance < self.tracking_threshold:
                                ...
                                # Smoothly update box size with more weight on new detection
                                ...
                                obj['kalman'].x[2] = np.clip(obj['kalman'].x[2], -0.5, 0.5)  # Relaxed limit vx
                                obj['kalman'].x[3] = max(obj['kalman'].x[3], 0.5)  # Increased min vy
                        ...
                        if not is_matched:
                            ...
                            kf.x = np.array([[center_x], [center_y], [0], [5.0]])
        ...
        for track_id, obj in list(tracked_objects.items()):
            ...
            # Predict next state for every object
            kf.predict()
            new_center_x, new_center_y = kf.x[0, 0], kf.x[1, 0]
            vx, vy = kf.x[2, 0], kf.x[3, 0]
            # Constrain velocities
            kf.x[2] = np.clip(vx, -0.5, 0.5)  # Relaxed limit x-velocity
            kf.x[3] = max(vy, 0.5)  # Ensure faster positive y-velocity
