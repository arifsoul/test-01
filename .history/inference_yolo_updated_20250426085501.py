...
tracked_objects = {}  # Store tracked objects: {track_id: {'center': (x, y), 'class': cls, 'confidence': conf, 'box': (x1, y1, x2, y2), 'kalman': KalmanFilter, 'last_seen': frame_count, 'box_size': (w, h)}}
...
def init_kalman_filter(fps=30):
    """Initialize Kalman Filter for tracking downward motion, synchronized with video FPS."""
    kf = KalmanFilter(dim_x=4, dim_z=2)  # State: [x, y, vx, vy], Measurement: [x, y]
    dt = 1.0 / fps  # Time step based on FPS
    kf.F = np.array([[1, 0, dt, 0],
                     [0, 1, 0, dt],
                     [0, 0, 1, 0],
                     [0, 0, 0, 1]])  # State transition matrix with FPS-based velocity scaling
    kf.H = np.array([[1, 0, 0, 0],
                     [0, 1, 0, 0]])  # Measurement matrix
    kf.R = np.eye(2) * 2.0  # Reduced measurement noise for faster response
    kf.P = np.eye(4) * 100  # Initial state covariance
    # Process noise scaled by dt, increased for vy to allow faster adaptation
    kf.Q = np.diag([0.01 * dt, 0.01 * dt, 0.001 * dt, 0.1 * dt])
    # Initialize with higher vy for faster downward motion
    kf.x = np.array([[0], [0], [0], [3.0]])  # Initial state: [x=0, y=0, vx=0, vy=3]
    return kf
...
class SimpleFruitDetectionGUI(QMainWindow):
    def __init__(self):
        ...
        self.max_frames_missing = 30  # Max frames an object can be missing
    ...
    def init_ui(self):
        ...
        for idx, (cls_id, cls_info) in enumerate(class_map.items()):
            ...
            color = (cls_info['color'][2], cls_info['color'][1], cls_info['color'][0])  # (R, G, B) -> (B, G, R)
    ...
    def start_detection(self):
        ...
        for obj in tracked_objects.values():
            obj['kalman'] = init_kalman_filter(self.video_fps)
    ...
    def update_frame(self):
                ...
                if results[0].boxes.id is not None:
                    ...
                    for box, track_id, cls, conf in zip(boxes, track_ids, classes, confidences):
                        ...
                        for existing_id, obj in tracked_objects.items():
                            ...
                            if distance < self.tracking_threshold:
                                ...
                                obj['kalman'].update(np.array([[center_x], [center_y]]))
                                # Smoothly update box size
                                prev_width, prev_height = obj['box_size']
                                new_width = 0.7 * prev_width + 0.3 * box_width
                                new_height = 0.7 * prev_height + 0.3 * box_height
                                ...
                                obj['kalman'].x[2] = np.clip(obj['kalman'].x[2], -0.1, 0.1)  # Limit vx
                                obj['kalman'].x[3] = max(obj['kalman'].x[3], 0.5)  # Ensure vy >= 0.5
                                new_tracked_objects[existing_id] = obj
                                break

                        if not is_matched:
                            # Initialize new object
                            kf = init_kalman_filter(self.video_fps)
                            kf.x = np.array([[center_x], [center_y], [0], [3.0]])
                    ...
                    for track_id, obj in new_tracked_objects.items():
                        if track_id not in detected_ids and self.frame_count - obj['last_seen'] <= self.max_frames_missing:
                            # Predict position for unmatched objects
                            obj['kalman'].predict()
                            pred_x, pred_y = obj['kalman'].x[0, 0], obj['kalman'].x[1, 0]
                            obj['kalman'].x[2] = np.clip(obj['kalman'].x[2], -0.1, 0.1)  # Limit vx
                            obj['kalman'].x[3] = max(obj['kalman'].x[3], 0.5)  # Ensure vy >= 0.5
                            obj['center'] = (pred_x, pred_y)
        ...
        # Update positions of tracked objects
        for track_id, obj in list(tracked_objects.items()):
            cls = obj['class']
            kf = obj['kalman']
            box_width, box_height = obj['box_size']
            
            # Predict next state (already done for unmatched objects)
            if self.frame_count % self.process_frame_interval != 0 or track_id in tracked_objects:
                kf.predict()
                new_center_x, new_center_y = kf.x[0, 0], kf.x[1, 0]
                vx, vy = kf.x[2, 0], kf.x[3, 0]
                # Constrain velocities
                kf.x[2] = np.clip(vx, -0.1, 0.1)  # Limit x-velocity
                kf.x[3] = max(vy, 0.5)  # Ensure positive y-velocity >= 0.5
                obj['center'] = (new_center_x, new_center_y)
            else:
                new_center_x, new_center_y = obj['center']
                vx, vy = kf.x[2, 0], kf.x[3, 0]
