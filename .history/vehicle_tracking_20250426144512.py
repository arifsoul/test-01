...
def generate_html_report(data, le_type, le_location):
    ...
    # Clean data to ensure JSON-serializable values
    def clean_string(s):
        if s is None:
            return ""
    ...
    cluster_chart_data = [{'cluster': clean_string(k), 'count': v} for k, v in cluster_counts.items() if k != -1]
    ...
    try:
        ...
        # Save rendered output (if possible) for debugging
        try:
