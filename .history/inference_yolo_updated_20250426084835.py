...
def init_kalman_filter(fps=30):
    ...
    kf.R = np.eye(2) * 5.0  # Measurement noise covariance
    kf.P = np.eye(4) * 100  # Initial state covariance
    # Process noise scaled by dt to account for frame rate
    kf.Q = np.diag([0.01 * dt, 0.01 * dt, 0.001 * dt, 0.05 * dt])
    # Initialize with small positive vy for downward motion
    kf.x = np.array([[0], [0], [0], [1.0]])  # Initial state: [x=0, y=0, vx=0, vy=1]
    return kf
