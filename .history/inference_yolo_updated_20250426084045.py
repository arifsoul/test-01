...
def init_kalman_filter():
    """Initialize Kalman Filter for tracking primarily downward motion."""
    ...
    # Adjust process noise to favor y-direction motion and limit x-direction changes
    kf.Q = np.diag([0.01, 0.01, 0.001, 0.05])  # Lower noise for vx, higher for vy
...
class SimpleFruitDetectionGUI(QMainWindow):
    ...
    def update_frame(self):
                ...
                if results[0].boxes.id is not None:
                    ...
                    for box, track_id, cls, conf in zip(boxes, track_ids, classes, confidences):
                        ...
                        for existing_id, obj in tracked_objects.items():
                            ...
                            if distance < self.tracking_threshold:
                                ...
                                if conf > obj['confidence']:
                                    obj['class'] = cls
                                    obj['confidence'] = conf
                                # Constrain vx to near zero and ensure vy > 0
                        ...
                        if not is_same_position:
                            ...
                            kf.x = np.array([[center_x], [center_y], [0], [1.0]])  # Initialize with downward vy
                    ...
                    # Update tracked_objects, remove stale objects
                    tracked_objects = {
        ...
        # Update positions of tracked objects using Kalman Filter
        for track_id, obj in list(tracked_objects.items()):
            ...
            # Constrain velocity: limit vx and ensure vy > 0
            kf.x[2] = np.clip(vx, -0.1, 0.1)  # Limit x-velocity
            kf.x[3] = max(vy, 0.1)  # Ensure positive y-velocity (downward)
