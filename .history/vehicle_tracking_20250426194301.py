...
def generate_html_report(data, le_type, le_location):
    ...
    # Debug: Print JSON strings
    print("type_chart_json:", type_chart_json)
    print("location_chart_json:", location_chart_json)
    print("cluster_chart_json:", cluster_chart_json)
    
    # Use string formatting to embed JSON directly
    html_content = f"""
    <!DOCTYPE html>
    <html lang="en">
    <head>
    ...
    <body class="bg-gray-100 p-6">
        ...
        <script>
            const {{ Component: RechartsComponent, ...Recharts }} = window.Recharts || {{}};
            ...
            class ErrorBoundary extends React.Component {{
                ...
                render() {{
                    if (this.state.hasError) {{
                        return <div className="error">Error rendering chart: {{this.state.error.toString()}}</div>;
                    }}
            ...
            const App = () => (
                <div className="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md">
                        ...
                        <div className="chart-container">
                            ...
                            <Recharts.ResponsiveContainer width="100%" height={{300}}>
                                <Recharts.BarChart data={{locationChartData}}>
                                    <Recharts.CartesianGrid strokeDasharray="3 3" />
                                    <Recharts.XAxis dataKey="location" tick={{ fontSize: 12 }} />
                                    <Recharts.YAxis />
                                    <Recharts.Tooltip />
                                    <Recharts.Legend />
                    ...
                    <div className="mt-6">
                        ...
                        <p>
                            The location <strong>{clean_string(location_chart_data[0]['location'] if location_chart_data else 'Unknown')}</strong> appears most frequently,
                            with <strong>{location_chart_data[0]['count'] if location_chart_data else 0}</strong> events, indicating a potential parking hotspot.
                        </p>
                    </div>
                    <div className="mt-6">
            ...
            try {{
                ...
                errorDiv.textContent = 'Error rendering application: ' + e.toString();
            }}
        </script>
    </body>
    </html>
