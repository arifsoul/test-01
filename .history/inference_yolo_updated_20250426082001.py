...
tracked_objects = {}  # Store tracked objects: {track_id: {'center': (x, y), 'class': cls, 'confidence': conf, 'box': (x1, y1, x2, y2), 'kalman': KalmanFilter, 'last_seen': frame_count, 'box_size': (width, height)}}
...
def init_kalman_filter(initial_x, initial_y):
    ...
    kf.R *= 3  # Trust YOLO detections more
    kf.P *= 1000  # Initial state covariance
    kf.Q *= 0.7  # Allow some non-linear movement
    kf.x = np.array([[initial_x], [initial_y], [0], [0]])  # Initial state
    return kf
...
class SimpleFruitDetectionGUI(QMainWindow):
    def __init__(self):
        ...
        self.line_y_ratio = 0.6  # Counting line at 60% of frame height
        self.process_frame_interval = 15  # Detect every 30 frames (1 frame per second at 30 FPS)
        self.model = None
        self.video_fps = 30  # Default FPS, updated from video
        self.total_frames = 0
        self.duration = 0
        self.average_fps = 0
    ...
    def init_ui(self):
        ...
        self.line_y_input = QLineEdit("60")
    ...
    def update_frame(self):
        ...
        if self.frame_count % self.process_frame_interval == 0 and self.model:
            print(f"Frame {self.frame_count}: Running YOLO inference")
            try:
                with torch.no_grad():
                    ...
                    for box, track_id, cls, conf in zip(boxes, track_ids, classes, confidences):
                        ...
                        for existing_id, obj in tracked_objects.items():
                            ...
                            if distance < self.tracking_threshold:
                                ...
                                if cls == obj['class']:
                                    # Same class: use the latest detection
                                    kf = init_kalman_filter(center_x, center_y)
                                ...
                                elif conf > obj['confidence']:
                                    # Different class, higher confidence: use the new detection
                                    kf = init_kalman_filter(center_x, center_y)
                                    new_tracked_objects[track_id] = {
                                        'center': center,
                                        'class': cls,
                                    ...
                                    obj['box_size'] = (box_width, box_height)
                                    new_tracked_objects[existing_id] = obj
                                break

                        if not is_same_position:
                            # New object: initialize Kalman Filter
                            kf = init_kalman_filter(center_x, center_y)
                            new_tracked_objects[track_id] = {
                                'center': center,
                                'class': cls,
        ...
        for track_id, obj in list(tracked_objects.items()):
            ...
            # Update bounding box using predicted center and stored box size
            ...
            cv2.putText(frame, f"ID: {track_id}, Conf: {conf:.2f}", (int(new_x1), int(new_y1) - 10),
        ...
        if frame is not None and frame.size > 0:
            cv2.line(frame, (0, int(line_y)), (int(width), int(line_y)), (0, 0, 255), 2)
        else:
