...
def fetch_data_from_snowflake():
    try:
        ...
        FROM TES_DB_RAW.RAW_LACAK.TR_TRACK
...
def preprocess_data(data):
    ...
    filtered_data = [
        row for row in data
        if row['UNIT'] and row['TYPE'] and row['DATE_TIME_BEGIN'] and row['DATE_TIME_END']
    ]
    ...
    for row in filtered_data:
        try:
            begin_date = datetime.datetime.strptime(row['DATE_TIME_BEGIN'], '%Y-%m-%d %H:%M:%S')
            end_date = datetime.datetime.strptime(row['DATE_TIME_END'], '%Y-%m-%d %H:%M:%S')
            processed_row = {
                'UNIT': row['UNIT'],
                'TYPE': row['TYPE'],
