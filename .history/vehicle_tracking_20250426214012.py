...
def generate_html_report(data, le_type, le_location):
    ...
    location_chart_data = [
        ...
        sorted(location_counts.items(), key=lambda x: x[1], reverse=True)[:3]  # Limit to top 3
    ]
    cluster_chart_data = [
        {"cluster": clean_string(str(k)), "count": v} for k, v in
        sorted(cluster_counts.items(), key=lambda x: x[1], reverse=True)[:5] if k != -1  # Limit to top 5
    ]
    ...
    <head>
        ...
        <style>
            ...
            .chart-container {{ margin: 20px 0; max-width: 600px; }}
            .error {{ color: red; font-weight: bold; }}
            .chart {{ width: 100%; height: 250px; }} /* Reduced height */
        </style>
    </head>
                ...
                try {{
                    ...
                    root.innerHTML = `
                        ...
                        <div id="location-chart" class="chart-container">
                            <h2 class="text-2xl font-semibold mb-2">Top 3 Locations</h2>
                            <canvas id="location-chart-canvas" class="chart"></canvas>
                        </div>
                        <div id="cluster-chart" class="chart-container">
                            <h2 class="text-2xl font-semibold mb-2">Top 5 Cluster Distribution</h2>
                            <canvas id="cluster-chart-canvas" class="chart"></canvas>
                        </div>
                        <div class="mt-6">
                            <h2 class="text-2xl font-semibold mb-2">Interesting Fact</h2>
                            <p>
                                The location <strong>${location_chart_data[0]['location']}</strong> appears most frequently,
