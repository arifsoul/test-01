...
class SimpleFruitDetectionGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        video_panel = QWidget()
        video_layout = QHBoxLayout(video_panel)
        ...
        video_layout.addWidget(self.thumbnail_label, 1)
    ...
    def start_detection(self):
        ...
        self.progress_bar.setMaximum(self.total_framesшеб

System: ### Penjelasan Perubahan dan Peningkatan

Berikut adalah rincian perubahan yang dilakukan pada kode untuk memenuhi permintaan Anda:

1. **Menampilkan FPS**:
   - Ditambahkan label `self.fps_label` untuk menampilkan FPS di panel kontrol.
   - FPS dihitung setiap 10 frame di metode `update_frame` menggunakan waktu yang telah berlalu sejak `start_time`. Nilai ini diperbarui di antarmuka dengan format `FPS: {fps:.2f}`.

2. **Thumbnail di Samping Video**:
   - Tata letak video dan thumbnail diubah dari `QVBoxLayout` (vertikal) menjadi `QHBoxLayout` (horizontal) di dalam `video_panel`. Sekarang thumbnail selalu berada di samping video, baik saat inference berjalan maupun tidak.
   - Proporsi tata letak diatur dengan `addWidget(self.video_label, 3)` dan `addWidget(self.thumbnail_label, 1)` untuk memastikan video mendapatkan ruang lebih besar dibandingkan thumbnail.

3. **Pemilihan Video dengan String Path dan Browse File**:
   - Ditambahkan tombol "Browse" (`self.browse_btn`) yang terhubung ke metode `browse_video`.
   - Metode `browse_video` menggunakan `QFileDialog.getOpenFileName` untuk membuka dialog pemilihan file, dengan filter untuk file video (`*.mp4 *.avi *.mov`).
   - Ketika file dipilih, path-nya diatur ke `self.video_input`, dan thumbnail diperbarui secara otomatis melalui sinyal `textChanged`.
   - Input teks (`self.video_input`) tetap mendukung entri manual path video, dengan thumbnail yang diperbarui saat teks berubah.

4. **Fungsi Inti yang Dipertahankan**:
   - Inference setiap 30 frame menggunakan `self.process_frame_interval`.
   - Pelacakan objek dengan YOLO (`persist=True`) dan penghitungan saat objek melewati garis penghitung (`line_y`) tetap sama.
   - Klasifikasi kualitas buah (`Ripe`, `Unripe`, `OverRipe`, `Rotten`, `EmptyBunch`) sesuai dengan `class_map`.

5. **Peningkatan Antarmuka**:
   - Tata letak panel kontrol disesuaikan agar lebih rapi, dengan tombol browse ditempatkan di samping input path video menggunakan `QHBoxLayout`.
   - Gaya visual dipertahankan dengan warna modern, tombol responsif, dan progress bar yang konsisten.
   - Label FPS ditambahkan di bawah tombol reset untuk visibilitas yang baik.

### Cara Menjalankan
1. Pastikan dependensi terinstal: `opencv-python`, `torch`, `ultralytics`, `PyQt5`.
2. Simpan model YOLO di folder `model/22K-5 Margaretkan model di folder `model/22K-5-M.pt`.
3. Simpan video di folder `video/conveyor.mp4` (atau pilih video lain melalui tombol browse).
4. Jalankan skrip: `python inference_yolo.py`.

### Catatan
- FPS dihitung berdasarkan waktu pemrosesan frame, memberikan gambaran performa real-time.
- Thumbnail tetap terlihat di samping video, bahkan saat inference tidak berjalan, memberikan pratinjau yang konsisten.
- Dialog pemilihan file memudahkan pengguna untuk memilih video tanpa perlu mengetik path secara manual.
- Jika video tidak valid, pesan error ditampilkan di `video_label`.

### Artifact
Berikut adalah kode lengkap yang telah diperbarui, dibungkus dalam tag `<xaiArtifact>` sesuai pedoman:

<xaiArtifact artifact_id="f9bbc951-f40c-4f3a-832b-f94d011be6c3" artifact_version_id="3cdf5f16-5439-47cb-be35-01aee67a974b" title="inference_yolo.py" contentType="text/python">
import cv2
import torch
from ultralytics import YOLO
from collections import defaultdict
