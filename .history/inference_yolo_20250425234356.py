...
tracked_objects = {}  # Store tracked objects: {track_id: {'center': (x, y), 'class': cls, 'confidence': conf, 'box': (x1, y1, x2, y2)}}

# Class map with icons and colors
class_map = {
    0: {'name': 'Ripe', 'icon': '🍎', 'color': (39, 174, 96)},      # Green
    1: {'name': 'Unripe', 'icon': '🍏', 'color': (241, 196, 15)},   # Yellow
    2: {'name': 'OverRipe', 'icon': '🍑', 'color': (230, 126, 34)}, # Orange
    3: {'name': 'Rotten', 'icon': '🥭', 'color': (192, 57, 43)},    # Red
    4: {'name': 'EmptyBunch', 'icon': '🌿', 'color': (127, 140, 141)} # Gray
}
...
class SimpleFruitDetectionGUI(QMainWindow):
    ...
    def update_frame(self):
        ...
        # Update tracked objects and check counting line
