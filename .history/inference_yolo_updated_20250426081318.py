...
def init_kalman_filter(initial_x, initial_y):
    ...
    kf.R *= 5  # Measurement noise covariance (trust YOLO detections)
    kf.P *= 1000  # Initial state covariance
    kf.Q *= 0.5  # Process noise covariance (allow some non-linear movement)
                ...
                if results[0].boxes.id is not None:
                    ...
                    for box, track_id, cls, conf in zip(boxes, track_ids, classes, confidences):
                        ...
                        for existing_id, obj in tracked_objects.items():
                            ...
                            if distance < self.tracking_threshold:
                                ...
                                if cls != obj['class'] and conf > obj['confidence']:
                                    # Update class, confidence, and Kalman Filter if new detection is better
                        ...
                        if not is_same_position:
                            # Initialize new Kalman Filter for new object
                            kf = init_kalman_filter(center_x, center_y)
                            new_tracked_objects[track_id] = {
                                'center': center,
                                'class': cls,
